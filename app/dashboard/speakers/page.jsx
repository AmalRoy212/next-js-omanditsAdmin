...
import { fetchAllSpeakers, fetchSpeakers } from '@/app/lib/data';
import Link from 'next/link'
import Pagination from '@/app/ui/dashboard/pagination/Pagination';
import DeleteButton from '@/app/ui/dashboard/users/delete/DeleteButton';

async function Speakers({searchParams}) {

  const q  = searchParams?.q || ""
  const page  = searchParams?.page || 1;
  const { count, speakers } = await fetchSpeakers(q, page); 
  const  allSpekers  = await fetchAllSpeakers();

  const plainObject = JSON.parse(JSON.stringify(allSpekers));
  const plainAllDele = JSON.parse(JSON.stringify(allSpekers));
